...
// Ecm = Ecommerce. Nome Dado Por convenção, fazendo referencia ao projeto. (abordagem Machine, por Database ou Máquina)
public class EcmMachineMultiTenantConnectionProvider implements
    ...
    public void start() {
        ...
        /*
            Idéia para pegar as configuracoes de tenant de uma tabela de um banco de dados a parte:
            DriverManagerConnectionProviderImpl cp = new DriverManagerConnectionProviderImpl();
            cp.configure(this.properties);
            cp.getConnection();
         */

        configurarTenant("algaworks_ecommerce",
                "jdbc:mysql://localhost/algaworks_ecommerce?createDatabaseIfNotExist=true&useTimezone=true&serverTimezone=UTC",
                "root",
                "1234567");
