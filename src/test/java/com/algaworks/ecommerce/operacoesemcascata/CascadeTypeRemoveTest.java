...
//    @Test
    public void removerItensOrfaos() {
        ...
        pedido.getItens().clear(); // orphanRemoval = true, só funciona com cascade PERSIST junto.
    ...
    public void removerPedidoEItens() {
        ...
        entityManager.getTransaction().begin();
        //pedido.getItens().forEach(item -> entityManager.remove(item)); // sem precisar de CascadeType.REMOVE, usa dessa forma.
...
//    @Test
    public void removerItemPedidoEPedido() {
        ...
        Pedido pedidoVerificacao = entityManager.find(
                Pedido.class, itemPedido.getPedido().getId());
        Assert.assertNull(pedidoVerificacao);
    }
