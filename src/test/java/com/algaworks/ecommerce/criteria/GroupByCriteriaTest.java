...
//                "having sum(ip.precoProduto) > 100";
        ...
        // só pode utilizar no having funções de agregação ou que tenham sido utilizadas no group by
//        criteriaQuery.having(
//            criteriaBuilder.greaterThan(joinProdutoCategoria.get(Categoria_.id), 2)
//        );
...
//                " group by year(p.dataCriacao), month(p.dataCriacao)";
        ...
        list.forEach(arr -> System.out.println(("Ano/Mês: " + arr[0] + ", Sum: " + arr[1])));
    }

    @Test
    public void agruparResultados03Exercicio() {
...
//                " group by c.id";
        ...
        TypedQuery<Object[]> typedQuery = entityManager.createQuery(criteriaQuery);
        List<Object[]> list = typedQuery.getResultList();
        list.forEach(arr -> System.out.println("Nome do Cliente: " + arr[0] + ", Sum: " + arr[1]));
    }
...
//                    criteriaBuilder.function("year", Date.class, joinPedido.get(Pedido_.dataCriacao)).as(java.sql.Date.class),
//                    criteriaBuilder.function("year", Date.class, criteriaBuilder.currentDate())
//            )
//        );
        ...
        TypedQuery<Object[]> typedQuery = entityManager.createQuery(criteriaQuery);
        List<Object[]> list = typedQuery.getResultList();
        list.forEach(arr -> System.out.println("Nome categoria: " + arr[0] + ", Sum: " + arr[1]));
    }
...
//        Join<Categoria, Produto> joinProduto = root.join(Categoria_.produtos); //inner join, so tras com correspondencia dos dois lados (
        Join<Categoria, Produto> joinProduto = root.join(Categoria_.produtos, JoinType.LEFT); // vai trazer inclusive Com Produto Nulo, priorizando a tabela a esquerda (categoria), vai ttrazer todas as categorias mesmo se nao tiver produto associado
        ...
        TypedQuery<Object[]> typedQuery = entityManager.createQuery(criteriaQuery);
        List<Object[]> list = typedQuery.getResultList();
        list.forEach(arr -> System.out.println("Nome: " + arr[0] + ", Count: " + arr[1]));
    }
