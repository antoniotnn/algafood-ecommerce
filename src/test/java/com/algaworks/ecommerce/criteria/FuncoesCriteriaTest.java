...
public class FuncoesCriteriaTest extends EntityManagerTest {
    ...
    public void aplicarFuncaoNumero() {
        ...
        list.forEach(arr -> System.out.println(
            arr[0]
                + ", abs: " + arr[1] // valor absoluto (sem sinal)
                + ", mod: " + arr[2] // resto da divisao
                + ", sqrt: " + arr[3] // raiz quadrada
        ));
    }
    ...
    public void aplicarFuncaoData() {
        ...
        //Verifica se o primeiro argumento do between está entre o seu segundo e terceiro argumentos, ou seja,
        //verifica se a a data de hoje , está dentro da data de criacao do pedido e da data de seu vencimento, ou seja, se o Boleto nao venceu.
        criteriaQuery.where(
    ...
    public void aplicarFuncaoString() {
        ...
        criteriaQuery.multiselect(
            ...
            criteriaBuilder.locate(root.get(Cliente_.nome), "a"), // retorna posição de "a" no atributo nome, tal como indexOf do Java
            criteriaBuilder.substring(root.get(Cliente_.nome), 1, 2),// params: em qual atributo buscar, a partir de qual posicao, quantos caracteres buscar a partir do inicio
            criteriaBuilder.lower(root.get(Cliente_.nome)),
            criteriaBuilder.upper(root.get(Cliente_.nome)),
            criteriaBuilder.trim(root.get(Cliente_.nome)) //remove espaços no inicio e no fim
        );

//        criteriaQuery.where(criteriaBuilder.equal(criteriaBuilder.substring(root.get(Cliente_.nome), 1, 1), "M"));
